/// <reference path="../types/TypedSteamUser.d.ts"/>
/**
 * @returns {TypedSteamUser & SteamUser}
 */
function asTypedSteamUser(steamUser) {
    return steamUser;
}
...
const client = asTypedSteamUser(new SteamUser());
...
SteamUser.prototype.chatMessage = function (steamID, message) {
    ...
    const friend = require('../app/handler/friends').getFriend(steamID);
...
function webSessionEvent (sessionID, cookies) {
    ...
    require('./community').setCookies(cookies);
...
function errorEvent (err) {
    if (err.eresult === SteamUser.EResult.LogonSessionReplaced) {
        ...
        if (sessionReplaceCount > 0) {
            ...
            handlerManager.getHandler().shutdown(err, false, true);
            return;
        }
