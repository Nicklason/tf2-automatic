...
function friendMessage(steamID, message) {
	...
	if (command == 'how2trade') {
	...
	} else if (command == 'reply' && Automatic.isOwner(steamID64)) {
		...
		const reply = message.substr(message.toLowerCase().indexOf(recipient) + 1);
		client.chatMessage(recipient, 'Message from owner (' + steamID64 + '): ' + reply);
		client.chatMessage(steamID64, 'Message has been sent to ' + recipient + '.');
		...
		for (let i = 0; i < owners.length; i++) {
			...
			client.chatMessage(owners[i], steamID64 + ' replied with "' + reply + '".');
		}
